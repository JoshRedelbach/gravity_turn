...
def find_initial_kick_angle(throttle):
    lower_bound_angle = -np.deg2rad(60)
    upper_bound_angle = -np.deg2rad(0.5)
    res = minimize_scalar(kick_angle_objective, bounds=(lower_bound_angle, upper_bound_angle), method='bounded', args=(throttle,))
    return res.x

alpha = find_initial_kick_angle(1.1)
print("Initial kick angle: ", np.rad2deg(alpha))
